...
public class Jack implements KeyListener {
	...
	GraphicsEnvironment ge = 
        ...
        frame = new JFrame("UB :: Programacio I- Mision Espacial-");
        canvas = new Canvas();
    ...
    private void actualitzarJoc() {
        ...
        ArmaHeroi armaHeroi = (ArmaHeroi)joc.getArmaHeroi();

        if (heroi.isMort()) {
            logica.setEstat(LogicaJoc.EstatJoc.GAMEOVER);
        } else if (heroi.haTrobatElsCristalls()) {
            logica.setEstat(LogicaJoc.EstatJoc.WIN);
        } else {        
            heroi.actualitzar(context);
            armaHeroi.actualitzar(context);
            ...
            for (Actor actor : h.getActors()) {
                ...
                if (actor.getEstat() == Constants.ESTAT_ACTIU && 
                        armaHeroi.getEstat() == Constants.ESTAT_ACTIU && 
                        armaHeroi.getLimits().intersects(actor.getLimits())) {
                    Colisio colisio = new Colisio(armaHeroi);
                    actor.tractarColisio(colisio);
                }
            }        
        }
